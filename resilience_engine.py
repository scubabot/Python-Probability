...
- Loads MATLAB workspace data (raw coordinates + ground truth)
- Trains a Gaussian Process in Python
- Reconstructs the environmental field on the same grid
- Visualizes actual vs learned GP + uncertainty

Next steps:
- Replace MATLAB-provided samples with Python-generated sensing
- Add meeting + W-MSR consensus
- Add informative planning (MI-based)
"""
...
# -----------------------------
# 1) Load MATLAB workspace (WINDOWS SAFE)
# -----------------------------
...
# Extract data from MATLAB struct
X_train = g["xi"]                    # (43, 2)
y_train = g["yi"].reshape(-1)        # (43,)
...
plt.scatter(
    X_train[:, 0] * 0 + (GRID_RES // 2),  # visual marker only
