# wmsr_paper_v5_4drones_camera_live.py
# 4 drones + 1 faulty (default), exactly 2 cycles:
#   Start -> Explore -> Meet(1) -> Share hypers (WMSR/Mean)
#        -> Explore -> Meet(2) -> Share hypers -> STOP
#
# + Live onboard camera view for EACH drone (OpenCV windows)
#
# Run:
#   (.venv) python wmsr_paper_v5_4drones_camera_live.py
#
# Notes:
# - Live view needs a display (X11). If you are on SSH headless, it won't show windows.
# - Press 'q' in any OpenCV window to stop.
...
def meeting_formation_pentagon(center_xyz: np.ndarray, num: int, radius: float) -> np.ndarray:
    """
    For 5 drones: pentagon around center.
    For other num: uses first num vertices.
    """
    angles = np.linspace(0, 2*np.pi, 6)[:-1]  # 5 angles
    offsets = np.c_[radius*np.cos(angles), radius*np.sin(angles), np.zeros_like(angles)]
    return center_xyz.reshape(1, 3) + offsets[:num].astype(np.float32)


def subareas_for_drones_5(XMIN, XMAX, YMIN, YMAX):
    """
    5 subareas: split into a 3x2 grid, use 5 cells.
    (For NUM=4, we just use the first 4.)
    """
    xs = np.linspace(XMIN, XMAX, 4)  # 3 columns
    ys = np.linspace(YMIN, YMAX, 3)  # 2 rows
    cells = []
    for r in range(2):
        for c in range(3):
            xmin, xmax = xs[c], xs[c+1]
            ymin, ymax = ys[r], ys[r+1]
            cells.append((float(xmin), float(xmax), float(ymin), float(ymax)))
    return cells[:5]
...
# ============================================================
# Onboard camera helpers (PyBullet virtual camera)
# ============================================================
def _quat_to_rotmat_xyzw(quat_xyzw):
    """PyBullet quat is (x,y,z,w)."""
    return np.array(p.getMatrixFromQuaternion(quat_xyzw), dtype=float).reshape(3, 3)


def drone_onboard_camera_rgb(
    pos,
    quat_xyzw,
    img_w=240,
    img_h=180,
    ...
    near=0.02,
    far=8.0,
    forward_cam_offset=0.06,
    up_cam_offset=0.02,
    pitch_deg=-10.0,
    renderer=p.ER_TINY_RENDERER,
):
    """
    Forward-facing onboard camera.
    - Uses body x-axis as forward direction (reasonable for CF2X in gym-pybullet-drones).
    """
    R = _quat_to_rotmat_xyzw(quat_xyzw)

    fwd = R[:, 0]  # body x in world
    up  = R[:, 2]  # body z in world

    cam_pos = np.array(pos, dtype=float) + forward_cam_offset * fwd + up_cam_offset * up

    pitch = np.deg2rad(pitch_deg)
    fwd_pitched = np.cos(pitch) * fwd - np.sin(pitch) * up
    target = cam_pos + 1.0 * fwd_pitched
    ...
    rgba = np.array(rgba, dtype=np.uint8).reshape(img_h, img_w, 4)
    rgb = rgba[:, :, :3]
    return rgb


# ============================================================
# Paper-style plotting (5-panel)
# ============================================================
...
def plot_paper_style_paths_5panel(
    outdir: Path,
    pos_hist: np.ndarray,
    phase_hist: list,
    cycle_hist: list,
    ...
    5 panels:
      (a)-(d): robot 1..4
      (e): replanning after meeting (all robots)
    """
    ...
    for i in range(NUM):
        Xp = drones_data[i]["X"]
        sensing_pts.append(Xp[::THIN_SENSE] if Xp.shape[0] > 0 else np.empty((0, 2), dtype=float))

    fig = plt.figure(figsize=(19, 4.4), dpi=240)
    axes = [fig.add_subplot(1, 5, i + 1) for i in range(5)]

    for r in range(min(4, NUM)):
        ax = axes[r]
    ...
    for i in range(NUM):
        if sensing_pts[i].shape[0] > 0:
            ax.scatter(sensing_pts[i][:, 0], sensing_pts[i][:, 1], s=6, c="red", alpha=0.35, marker=".")
    ...
    plt.savefig(outdir / "paper_style_paths_5panel.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    # ---- Stability knobs (avoid VS Code crash) ----
    GUI = False                 # This is PyBullet GUI; camera windows are OpenCV (separate)
    LOG_EVERY = 8               # log state at 30 Hz
    SENSE_EVERY = 4             # measurement at 60 Hz
    MAX_SAMPLES_PER_DRONE = 6000
    # ---------------------------------------------

    # ---- Live onboard camera settings ----
    CAM_ON = True
    CAM_EVERY = 12              # 240Hz/12 = 20 FPS
    CAM_W, CAM_H = 240, 180
    # Renderer: Tiny is reliable even if PyBullet GUI is off.
    # If you set GUI=True and want faster camera, change to p.ER_BULLET_HARDWARE_OPENGL.
    ...
    MAX_CYCLES = 2  # two meetings

    # explore
    EXPLORE_TIMEOUT_SEC = 55.0
    EXPLORE_MIN_SEC = 12.0
    WP_REACH_XY = 0.15
    WAYPOINT_STEP_EXPLORE = 0.03

    # meet
    Z_TOL = 0.28
    MEET_RADIUS_XY = 0.24
    MEET_SETTLE_SEC = 0.75
    MEET_TIMEOUT_SEC = 25.0
    WAYPOINT_STEP_MEET = 0.02
    ...
    # Fault model (1 faulty)
    faulty_set = {3}  # last drone for NUM=4
    ...
    # start positions (2x3 grid, take first 4)
    ...
    A = (base.reshape(1, 3) + starts)[:NUM]
    TARGET_RPY = np.zeros((NUM, 3), dtype=np.float32)

    SUBAREAS = subareas_for_drones_5(XMIN, XMAX, YMIN, YMAX)
    ...
    margin = 0.18
    max_r_x = min(MEETING_CENTER[0] - (XMIN + margin), (XMAX - margin) - MEETING_CENTER[0])
    max_r_y = min(MEETING_CENTER[1] - (YMIN + margin), (YMAX - margin) - MEETING_CENTER[1])
    max_fit = float(max(0.12, min(max_r_x, max_r_y)))
    FORM_R = float(min(0.30, 0.90 * max_fit))

    print("\n=== PAPER-LIKE RUN (v5, NUM=4 + LIVE CAM) ===")
    ...
    print(f"Formation radius FORM_R={FORM_R:.3f}")
    print("Live camera: CAM_EVERY =", CAM_EVERY, f"({CTRL_FREQ//CAM_EVERY} fps), size =", (CAM_W, CAM_H))
    ...
    for k in range(MAX_STEPS):
        ...
        if phase == "meet":
            if meet_subphase == "approach":
                ...
                meet_targets = meeting_formation_pentagon(meet_center, NUM, FORM_R)
        ...
        for i in range(NUM):
            ...
            if (k % SENSE_EVERY) == 0:
                ...
                if drones[i]["X"].shape[0] > MAX_SAMPLES_PER_DRONE:
                    drones[i]["X"] = drones[i]["X"][-MAX_SAMPLES_PER_DRONE:]
                    drones[i]["y"] = drones[i]["y"][-MAX_SAMPLES_PER_DRONE:]

        env.step(rpms)

        # ===== Live onboard camera windows (ALL drones) =====
        if CAM_ON and (k % CAM_EVERY == 0):
            quit_now = False
            for i in range(NUM):
                s = np.array(get_state_vector(env, i), dtype=np.float32)
                pos = s[0:3]
                quat = s[3:7]  # (x,y,z,w)

                rgb = drone_onboard_camera_rgb(
                    pos=pos,
                    quat_xyzw=quat,
                    img_w=CAM_W,
                    img_h=CAM_H,
                ...
                cv2.putText(
                    bgr,
                    f"Drone {i}" + (" (FAULTY)" if i in faulty_set else ""),
                    (8, 18),
                ...
                cv2.imshow(f"onboard_drone_{i}", bgr)

            key = cv2.waitKey(1) & 0xFF
            if key == ord("q"):
                print("\n[QUIT] 'q' pressed. Exiting loop.")
                break
        # ====================================================
        ...
        if phase == "explore":
            ...
            if done or timed_out:
                ...
                print("[DEBUG] samples per drone:", [int(drones[i]['X'].shape[0]) for i in range(NUM)])
                ...
                if GUI and meeting_text_id is not None:
                    try:
